...
def test_classifier(clf, dataset, feature_list, folds = 1000):
    ...
    label_names = df_temp.index.values
    ...
    for train_idx, test_idx in cv: 
        test_names = [label_names[i] for i in test_idx]
        features_train = []
        features_test  = []
        labels_train   = []
        labels_test    = []
        ...
        for prediction, truth, name in zip(predictions, labels_test, test_names):
    ...
    try:
        file_dir = './results_negative/'
        if not os.path.exists(file_dir):
            os.mkdir(file_dir)
        f = open(file_dir + 'false_negatives.txt', 'w')
        for e in false_n:
            f.write('%s\n' %e)
        f.close()
        f = open(file_dir + 'false_positives.txt', 'w')
        for e in false_p:
            f.write('%s\n' %e)
        f.close()                     
